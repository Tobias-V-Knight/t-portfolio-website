...
| `data/content.ts` | All copy and data | Every word on the site. Split three ways: `home` (professional), `about` (the person), `resume` (the record). Skills are two levels, `skills` (eight discipline groups) and `toolbox` (named technologies), plus `capabilities` (the long form). `projects` is ordered for display, case studies first in T's ranking, and `hasWindow` is the one switch that decides case study or archive entry: `caseStudyProjects` and `archiveProjects` are both derived from it. Placeholder copy is flagged here and the flag renders on screen |
...
| `windows/Work.tsx` | PORTFOLIO window | Finder list view (Name / Date Modified / Kind / Size), one icon row per project, in two groups split by an `ARCHIVE` divider. The case studies above it open a window, the archive entries below it open nothing and are plain rows rather than disabled buttons |
